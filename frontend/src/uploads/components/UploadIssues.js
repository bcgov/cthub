...
const UploadIssues = ({
  ...
  const errorHeading = () => {
    const missingHeadersError =
      groupedCriticalErrors?.Headers?.["Missing Headers"]?.ExpectedType;
    const missingWorksheetError =
      groupedCriticalErrors?.Spreadsheet?.["Missing Worksheet"]?.ExpectedType;
      ...
      <Box sx={{ p: 2, mb: 2 }}>
        <h2 style={{ marginBottom: "16px" }}>
          ...
          {totalIssueCount.criticalErrors >= 1
            ? `File upload failed - ${errorHeading()}`
