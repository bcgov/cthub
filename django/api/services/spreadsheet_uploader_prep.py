...
def prepare_hydrogen_fueling(df):
    ...
    for field in decimal_columns:
        try:
            df[field] = df[field].apply(lambda x: round(Decimal(x), 2) if pd.notnull(x) else None)
        except:
...
def prepare_ldv_rebates(df):
    replacements = {
        ...
        "Delivered": {'YES': True, 'Y': True, 'NO': False, 'N': False, 'OEM': False, 'INCENTIVE_FUNDS_AVAILABLE': False},
