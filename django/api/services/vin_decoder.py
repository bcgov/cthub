...
def decoder():
    ...
    vin_queryset = IcbcRegistrationData.objects.values_list("vin", flat=True).order_by(
        "vin"
    )
    ...
    for each in pages:
        ...
        for vin in page.object_list:
            index += 1
            query_requests.update({"query_request_" + str(index): {"vin": vin}})
        ...
        decoder_query_object = {
            "decoder_settings": {
                ...
                "version": "7.2.0",
                "common_data": "on",
                "common_data_packs": {"basic_data": "on", "engines": "on"},
            },
