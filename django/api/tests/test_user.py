...
class TestUsers(TestCase):
    def setUp(self):
        ...
        self.admin_user_permission = UserPermission.objects.create(
            user=self.test_admin_user, permission=self.test_admin_permission
        )
        ...
        self.upload_user_permission = UserPermission.objects.create(
            user=self.test_upload_user, permission=self.test_upload_permission
        )

    def test_get_user_list(self):
        self.assertTrue(User.objects.filter(idir="test_user").exists())
        request = self.factory.get("/api/users/list")
        request.META = {"HTTP_AUTHORIZATION": {"idir": "test_user"}}
        ...
        request_uploader.META = {"HTTP_AUTHORIZATION": {"idir": "test_upload_user"}}
        ...
        request_admin.META = {"HTTP_AUTHORIZATION": {"idir": "test_admin_user"}}
        response = self.view(request_admin)
        self.assertEqual(response.status_code, 200)  # OK status code

    def test_not_authenticated_user(self):
        request = self.factory.get("/api/users/list")
        request.META = {"HTTP_AUTHORIZATION": {"idir": "test"}}
