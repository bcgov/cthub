...
class UploadTests(TestCase):
    ...
    def test_wrong_cell_types(self):
        ...
        df.to_excel(
            excel_writer=excel_buffer, sheet_name="TOP OTHER TRANSACTIONS", index=False
        )
        excel_buffer.seek(0)
        ...
        self.assertIn(
            "Row 1: Incorrect type for 'application_city_fuel'", response["errors"][1]
        )
    ...
    def test_missing_worksheet(self):
        ...
        excel_buffer = io.BytesIO()
        df.to_excel(
            excel_writer=excel_buffer, sheet_name="Wrong Sheet Name", index=False
        )
        excel_buffer.seek(0)
        ...
        self.assertFalse(response["success"])
        self.assertIn(
            "Worksheet named 'TOP OTHER TRANSACTIONS' not found", response["errors"][0]
        )
    ...
    def test_successful_upload(self):
        ...
        self.assertIn(
            "All 1 records successfully inserted out of 1.", response["message"]
        )
